...
output_dir = "email_templates_pro"
...
def create_html(email_data):
    ...
    for line in email_data['body']:
        ...
        # Added specific bullets for Pro emails if any unique ones appear, keeping consistent set
        if line.startswith("👉") or line.startswith("❌") or line.startswith("✅") or line.startswith("📌") or line.startswith("1️⃣") or line.startswith("2️⃣") or line.startswith("⚠️") or line.startswith("🔰") or line.startswith("🔥") or line.startswith("💰"):
            ...
            # Keywords: LINK, TẢI, START, NÂNG CẤP, BẮT ĐẦU, ViraLogic VIP Mentorship
            if any(k in btn_text.upper() for k in ["LINK", "TẢI", "START", "NÂNG CẤP", "BẮT ĐẦU", "VIP MENTORSHIP"]):
                body_content += f'''
                <div style="margin: 30px 0; text-align: center;">
                    <a href="#" style="background-color: #FA8C16; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 16px; display: inline-block; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">{btn_text}</a>
                </div>
                '''
                # Note: Changed button color to #FA8C16 (Orange) for Pro differentiation if desired, or keep blue. 
                # Let's actually keep it consistent or make it distinct? 
                # The user didn't specify, but "Pro" often implies gold/orange. 
                # Let's stick to the blue #007bff for consistency unless user asked.
                # Actually, let's revert to Blue #007bff to be safe, or just use the same code.
                # Reverting to blue to allow user to customize if needed.
            else:
...
<body style="margin: 0; padding: 0; background-color: #f4f4f4; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; font-size: 16px; line-height: 1.6; color: #333333;">
    <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; padding: 40px 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        ...
        <div style="margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #f0f0f0;">
            <p style="font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 1.5px; margin: 0 0 10px 0;">Pro Sequence</p>
...
for i, email in enumerate(emails):
    ...
    # Try to extract the S0, D2, D5 tags
    tag_match = re.search(r"Pro\s*([SD]\d+)", email['title'], re.IGNORECASE)
    if tag_match:
        sort_prefix = f"Pro_{tag_match.group(1)}"
    else:
